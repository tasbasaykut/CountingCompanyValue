    ...
    with st.spinner('Piyasa verileri, rakiplerin çarpanları ve döviz kurları çekiliyor...'):
        try:
            ...
            if not guncel_fiyat:
                ...
                # 1. DCF Arayüzü (GÜNCELLENEN KISIM)
                ...
                if fcf_baslangic is not None and hisse_adedi:
                    if fcf_baslangic < 0:
                        ...
                        st.error("⚠️ **Matematiksel Hata:** İskonto Oranı (WACC), Kalıcı Büyüme Oranından büyük olmalıdır. Lütfen sol menüdeki varsayımları düzeltin.")
                    else:
                        ...
                        col2.metric("Tahmini Değer (DCF)", f"${dcf_usd:,.2f}", f"%{((dcf_usd - guncel_fiyat_usd)/guncel_fiyat_usd)*100:.1f} Potansiyel")
                else:
                ...
                if eps and sektor_fk:
                    ...
                    col1, col2 = st.columns(2)
                    col1.metric("Borsadaki Aktif Değer", f"${guncel_fiyat_usd:,.2f}")
                    col2.metric("Tahmini Değer (F/K)", f"${fk_usd:,.2f}", f"%{((fk_usd - guncel_fiyat_usd)/guncel_fiyat_usd)*100:.1f} Potansiyel")
                else:
                ...
                if ebitda and hisse_adedi and sektor_fd_favok:
                    ...
                    col1, col2 = st.columns(2)
                    col1.metric("Borsadaki Aktif Değer", f"${guncel_fiyat_usd:,.2f}")
                    col2.metric("Tahmini Değer (FD/FAVÖK)", f"${fd_favok_usd:,.2f}", f"%{((fd_favok_usd - guncel_fiyat_usd)/guncel_fiyat_usd)*100:.1f} Potansiyel")
                else:
